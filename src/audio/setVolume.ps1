...
$deviceEnum.GetDefaultAudioEndpoint([EDataFlow]::eRender, [ERole]::eMultimedia, [ref]$device) | Out-Null
...
for ($i = 0; $i -lt $count; $i++) {
    ...
    $control.GetProcessId([ref]$sessionProcessId) | Out-Null

    if ($sessionProcessId -eq $processId) {
        $volume = [ISimpleAudioVolume]$control
        $level = [math]::Min([math]::Max($volumePercent, 0), 100) / 100
        $volume.SetMasterVolume($level, [Guid]::Empty) | Out-Null
        break
    }
}
